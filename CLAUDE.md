...
- A post must exist in both locales with the same filename; the slug list is read from the `es` directory (`src/lib/blog.ts`)
- The featured post (blog page + home BlogPreview) is always the most recent by date (`getFeaturedPost` returns `posts[0]`); the blog grid excludes it by slug
